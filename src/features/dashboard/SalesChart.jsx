...
function SalesChart({ bookings, numDays }) {
  ...
  // this data is structured baesd on "fakeData" object above there.
  const data = allDates.map((date) => {
    return {
      label: format(date, "MMM d"),

      // Get me all the bookings that are created in this time interval and accumalate all their total Prices.
      totalSales: bookings
        .filter((booking) => isSameDay(date, new Date(booking.created_at)))
        .reduce((acc, curr) => acc + curr.totalPrice, 0),

      // Get me all the bookings that are created in this time interval and accumalate all their extra prices.
      extraSales: bookings
        .filter((booking) => isSameDay(date, new Date(booking.created_at)))
